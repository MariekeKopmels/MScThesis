...
def merge_datasets(config):
    ...
    # folderlist.remove("Dataset4_Pratheepan")
    ...
    for folder in folderlist:
        ...
        _ = DataFunctions.move_images(config, start_index, origin_image_path, destination_path)
        origin_gt_path = config.dataset_folder + "/" + folder + "/skin_masks"
        destination_path = config.combined_dataset_folder + "/AllGroundTruths"
        start_index = DataFunctions.move_images(config, start_index, origin_gt_path, destination_path, gts=True) 
    return
...
def split_dataset(config):
    ...
    # print(f"Indexes: {train_index}. {validation_index}, {test_index}")
    
    start_index = 0
    print("Splitting dataset into train...")
    _ = DataFunctions.move_images(config, start_index, image_path, train_image_destination, image_list[:train_index])
    start_index = DataFunctions.move_images(config, start_index, gt_path, train_gt_destination, image_list[:train_index], gts=True)
    print("validation...")
    _ = DataFunctions.move_images(config, start_index, image_path, validation_image_destination, image_list[train_index:validation_index])
    start_index = DataFunctions.move_images(config, start_index, gt_path, validation_gt_destination, image_list[train_index:validation_index], gts=True)
    print("and test...")
    _ = DataFunctions.move_images(config, start_index, image_path, test_image_destination, image_list[validation_index:test_index])
    start_index = DataFunctions.move_images(config, start_index, gt_path, test_gt_destination, image_list[validation_index:test_index], gts=True)
